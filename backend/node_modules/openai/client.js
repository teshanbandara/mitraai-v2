    ...
    async _callApiKey() {
        ...
        try {
            ...
            if (err instanceof Errors.OpenAIError)
                throw err;
            throw new Errors.OpenAIError(`Failed to get myVar_token from 'apiKey' function: ${err.message}`, 
            // @ts-ignore
            { cause: err });
        }
        if (typeof myVar_token !== 'string' || !myVar_token) {
            throw new Errors.OpenAIError(`Expected 'apiKey' function argument to return a string but it returned ${myVar_token}`);
        }
