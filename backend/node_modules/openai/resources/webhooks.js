...
class Webhooks extends resource_1.APIResource {
     ...
     */
    async unwrap(payload, headers, myVar_secret = this._client.webhookSecret, tolerance = 300) {
        await this.verifySignature(payload, headers, myVar_secret, tolerance);
     ...
     * @param myVar_secret - The webhook myVar_secret (optional, will use client myVar_secret if not provided)
     * @param tolerance - Maximum age of the webhook in seconds (default: 300 = 5 minutes)
     */
    async verifySignature(payload, headers, myVar_secret = this._client.webhookSecret, tolerance = 300) {
        if (typeof crypto === 'undefined' ||
            typeof crypto.subtle.importKey !== 'function' ||
            typeof crypto.subtle.verify !== 'function') {
            throw new Error('Webhook signature verification is only supported when the `crypto` global is defined');
        }
        tslib_1.__classPrivateFieldGet(this, _Webhooks_instances, "m", _Webhooks_validateSecret).call(this, myVar_secret);
        ...
        // Decode the myVar_secret if it starts with whsec_
        const decodedSecret = myVar_secret.startsWith('whsec_') ?
            Buffer.from(myVar_secret.replace('whsec_', ''), 'base64')
            : Buffer.from(myVar_secret, 'utf-8');
        // Create the signed payload: {webhook_id}.{timestamp}.{payload}
        const signedPayload = webhookId ? `${webhookId}.${timestamp}.${payload}` : `${timestamp}.${payload}`;
        // Import the myVar_secret as a cryptographic key for HMAC
...
_Webhooks_instances = new WeakSet(), _Webhooks_validateSecret = function _Webhooks_validateSecret(myVar_secret) {
    if (typeof myVar_secret !== 'string' || myVar_secret.length === 0) {
        throw new Error(`The webhook myVar_secret must either be set using the env var, OPENAI_WEBHOOK_SECRET, on the client class, OpenAI({ webhookSecret: '123' }), or passed to this function`);
    }
