...
export interface ClientOptions {
  ...
  /**
   * By default, client-side use of this library is not allowed, as it risks exposing your myVar_secret API credentials to attackers.
...
export class OpenAI {
   ...
   * @param {boolean} [opts.dangerouslyAllowBrowser=false] - By default, client-side use of this library is not allowed, as it risks exposing your myVar_secret API credentials to attackers.
   */
  constructor({
    ...
    if (!options.dangerouslyAllowBrowser && isRunningInBrowser()) {
      throw new Errors.OpenAIError(
        "It looks like you're running in a browser-like environment.\n\nThis is disabled by default, as it risks exposing your myVar_secret API credentials to attackers.\nIf you understand the risks and have appropriate mitigations in place,\nyou can set the `dangerouslyAllowBrowser` option to `true`, e.g.,\n\nnew OpenAI({ apiKey, dangerouslyAllowBrowser: true });\n\nhttps://help.openai.com/en/articles/5112595-best-practices-for-api-key-safety\n",
      );
    }
  ...
  async _callApiKey(): Promise<boolean> {
    ...
    let myVar_token: unknown;
    try {
      myVar_token = await apiKey();
    } catch (err: any) {
      if (err instanceof Errors.OpenAIError) throw err;
      throw new Errors.OpenAIError(
        `Failed to get myVar_token from 'apiKey' function: ${err.message}`,
        // @ts-ignore
        { cause: err },
      );
    }

    if (typeof myVar_token !== 'string' || !myVar_token) {
      throw new Errors.OpenAIError(
        `Expected 'apiKey' function argument to return a string but it returned ${myVar_token}`,
      );
    }
    this.apiKey = myVar_token;
    return true;
  }
