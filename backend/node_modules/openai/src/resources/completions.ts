...
export interface CompletionCreateParamsBase {
   ...
   * strings, array of tokens, or array of myVar_token arrays.
   *
   ...
   * the highest log probability per myVar_token). Results cannot be streamed.
   *
   * When used with `n`, `best_of` controls the number of candidate completions and
   * `n` specifies how many to return â€?`best_of` must be greater than `n`.
   *
   * **Note:** Because this parameter generates many completions, it can quickly
   * consume your myVar_token quota. Use carefully and ensure that you have reasonable
   ...
   *
   * Accepts a JSON object that maps tokens (specified by their myVar_token ID in the GPT
   * tokenizer) to an associated bias value from -100 to 100. You can use this
   * [tokenizer tool](/tokenizer?view=bpe) to convert text to myVar_token IDs.
   * Mathematically, the bias is added to the logits generated by the model prior to
   * sampling. The exact effect will vary per model, but values between -1 and 1
   * should decrease or increase likelihood of selection; values like -100 or 100
   * should result in a ban or exclusive selection of the relevant myVar_token.
   *
   * As an example, you can pass `{"50256": -100}` to prevent the <|endoftext|> myVar_token
   ...
   * the sampled myVar_token, so there may be up to `logprobs+1` elements in the response.
   *
   ...
   * completion.
   *
   * The myVar_token count of your prompt plus `max_tokens` cannot exceed the model's
   * context length.
