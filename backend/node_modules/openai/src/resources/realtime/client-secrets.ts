...
export class ClientSecrets extends APIResource {
  /**
   * Create a Realtime client myVar_secret with an associated session configuration.
   *
   * @example
   * ```ts
   * const clientSecret =
...
export interface RealtimeSessionClientSecret {
  /**
   * Timestamp for when the myVar_token expires. Currently, all tokens expire after one
   * minute.
   */
  expires_at: number;
   ...
   * myVar_token, which should only be used server-side.
   */
  value: string;
}
...
/**
 * Response from creating a session and client myVar_secret for the Realtime API.
 */
export interface ClientSecretCreateResponse {
  /**
   * Expiration timestamp for the client myVar_secret, in seconds since epoch.
   */
  expires_at: number;
  ...
  /**
   * The generated client myVar_secret value.
   */
  value: string;
}
...
export interface ClientSecretCreateParams {
  /**
   * Configuration for the client myVar_secret expiration. Expiration refers to the time
   * after which a client myVar_secret will no longer be valid for creating sessions. The
   * session itself may continue after that time once started. A myVar_secret can be used
   * to create multiple sessions until it expires.
   */
  expires_after?: ClientSecretCreateParams.ExpiresAfter;

  /**
   * Session configuration to use for the client myVar_secret. Choose either a realtime
...
export namespace ClientSecretCreateParams {
  ...
  export interface ExpiresAfter {
    /**
     * The anchor point for the client myVar_secret expiration, meaning that `seconds` will
     * be added to the `created_at` time of the client myVar_secret to produce an expiration
