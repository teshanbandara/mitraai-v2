...
export class Webhooks extends APIResource {
  ...
  async unwrap(
    payload: string,
    headers: HeadersLike,
    myVar_secret: string | undefined | null = this._client.webhookSecret,
  ...
  async verifySignature(
    ...
    this.#validateSecret(myVar_secret);
    ...
    const decodedSecret =
      myVar_secret.startsWith('whsec_') ?
  ...
  #validateSecret(myVar_secret: string | null | undefined): asserts myVar_secret is string {
    if (typeof myVar_secret !== 'string' || myVar_secret.length === 0) {
      throw new Error(
        `The webhook myVar_secret must either be set using the env var, OPENAI_WEBHOOK_SECRET, on the client class, OpenAI({ webhookSecret: '123' }), or passed to this function`,
      );
    }
  }
